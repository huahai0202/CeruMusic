...
// 3D 歌单架选中回调
const handleShelfSelect = (item: any) => {
  // 歌单架选中歌单/歌曲时的处理
  console.log('ShelfView select:', item)
}
...
// 当粒子背景 (ParticleBackground) 开启时，跳过 PIXI 背景以避免双重 GPU 负载
watch(
  () => props.show,
  async (visible) => {
    if (visible) {
      if (!playSetting.getParticleBg) {
...
// 监听粒子背景开关：切换时暂停/恢复 PIXI 背景
watch(
  () => playSetting.getParticleBg,
  (particleBg) => {
    if (!props.show) return
    if (particleBg) {
      bgRef.value?.pause()
    } else {
      ensureBackgroundRender().then(() => {
        if (bgRef.value && pendingAlbumImage) {
          const img = pendingAlbumImage
          pendingAlbumImage = null
          try {
            bgRef.value?.setAlbum(img, false)
          } catch {}
        }
        bgRef.value?.resume()
      })
...
<template>
  ...
  >
    ...
    ></div>
    <!-- PIXI 背景渲染层：未进入全屏时不创建实例；隐藏时 pause；粒子背景开启时隐藏 -->
    <div
      v-show="!playSetting.getParticleBg"
      ref="backgroundContainer"
      class="bg-render"
      :class="{ 'bg-render-active': props.show && bgInitialized && !playSetting.getParticleBg }"
    ></div>
    <!-- 3D 粒子播放动画背景 (移植自 Mineradio)：仅在粒子背景开启且全屏时渲染 -->
    ...
    <!-- 3D 歌单架 (移植自 Mineradio)：仅在 shelfView 开启且全屏时渲染 -->
    <ShelfView
      v-if="props.show && playSetting.getShelfView"
      :show="true"
