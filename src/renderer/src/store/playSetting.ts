...
export const playSetting = defineStore('playSetting', {
  state: () => ({
    ...
    particleBg: false, // 是否开启 3D 粒子播放动画背景 (移植自 Mineradio)
    lyricStage: false, // 是否开启 3D 歌词舞台 (移植自 Mineradio)
    shelfView: false // 是否开启 3D 歌单架视图 (移植自 Mineradio)
  }),
  getters: {
    ...
    getParticleBg: (state) => state.particleBg,
    getLyricStage: (state) => state.lyricStage,
    getShelfView: (state) => state.shelfView
  },
  actions: {
    ...
    setParticleBg(particleBg: boolean) {
      this.particleBg = particleBg
    },
    setLyricStage(lyricStage: boolean) {
      this.lyricStage = lyricStage
    },
    setShelfView(shelfView: boolean) {
