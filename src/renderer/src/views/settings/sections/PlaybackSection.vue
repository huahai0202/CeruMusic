...
const { isJumpLyric, bgPlaying, isAudioVisualizer, particleBg, lyricStage, shelfView } = storeToRefs(playSettingStore)
...
<template>
  <div class="settings-section">
    ...
    <div id="playback-performance" class="setting-group">
      ...
      <div class="setting-item">
        <div class="item-info">
          <div class="item-title">3D 粒子播放动画</div>
          <div class="item-desc">启用 3D 粒子舞台背景 (移植自 Mineradio)、含节拍驱动镜头与视觉预设、占用较高 GPU</div>
        </div>
        <t-switch v-model="particleBg" @change="playSettingStore.setParticleBg(particleBg)" />
      </div>

      <div class="setting-item">
        <div class="item-info">
          <div class="item-title">3D 歌词舞台</div>
          <div class="item-desc">启用 3D 歌词舞台 (移植自 Mineradio)、含星河粒子、卡拉OK进度填充、节拍发光、需先开启 3D 粒子播放动画</div>
        </div>
        <t-switch v-model="lyricStage" @change="playSettingStore.setLyricStage(lyricStage)" />
      </div>

      <div class="setting-item">
        <div class="item-info">
          <div class="item-title">3D 歌单架</div>
          <div class="item-desc">启用 PSP 风格 3D 歌单卡片轮播 (移植自 Mineradio)、含弧形排列与交互</div>
        </div>
        <t-switch v-model="shelfView" @change="playSettingStore.setShelfView(shelfView)" />
      </div>
